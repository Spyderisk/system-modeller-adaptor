...
async def store_state_report(conn: AsyncIOMotorClient, ssm_model_id: str,
    ...
    state.ssm_model_id = ssm_model_id
...
async def remove_state_reports(conn: AsyncIOMotorClient, ssm_model_id) -> int:
    result = await conn[database_name][state_report_collection].delete_many({"ssm_model_id": ssm_model_id})
...
async def get_newer_reports(conn: AsyncIOMotorClient, ssm_model_id, date: str) -> List[StateReportMessage]:
    date_time = dateutil.parser.parse(date)
    logger.debug(f"CRUD get state reports newer than {date_time}, ssm_model_id: {ssm_model_id}")
    reports = []
    cursor = conn[database_name][state_report_collection].find({"ssm_model_id": ssm_model_id, "created_at": { "$gt": date_time}}).sort("created_at", DESCENDING)
...
async def get_valid_reports(conn: AsyncIOMotorClient, ssm_model_id) -> List[StateReportMessageInDB]:
    logger.debug(f"CRUD get valid state reports for ssm_model_id: {ssm_model_id}")
    reports = []
    cursor = conn[database_name][state_report_collection].find({"ssm_model_id": ssm_model_id}).sort("created_at", DESCENDING)
    for document in await cursor.to_list(length=100):
        created_at = document["created_at"]
...
async def get_all_reports(conn: AsyncIOMotorClient, ssm_model_id) -> List[StateReportMessage]:
    reports = []
    cursor = conn[database_name][state_report_collection].find({"ssm_model_id": ssm_model_id})
    for document in await cursor.to_list(length=100):
...
async def get_expired_reports(conn: AsyncIOMotorClient, ssm_model_id) -> List[str]:
    logger.debug(f"CRUD remove expired reports for model {ssm_model_id}")
